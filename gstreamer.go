...
func (p *Pipeline) Reset() {
	C.gstreamer_pipeline_reset(p.pipeline)
}
...
func (e *Element) SetLocation(location string) {
	C.gstreamer_set_location(e.element, C.CString(location))
}

func (e *Element) SetBindPort(port int) {
	C.gstreamer_bind_port(e.element, C.int(port))
}

func (e *Element) SeIpProperties(host string, port int) {
	C.gstreamer_set_ip(e.element, C.CString(host), C.int(port))
}
