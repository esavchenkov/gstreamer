...
void gstreamer_pipeline_reset(GstPipeline *pipeline) {
...
void gstreamer_set_location(GstElement *element, char *filepath) {
    GObject *obj = G_OBJECT(element);
    g_object_set(obj, "location", filepath, NULL);
}

void gstreamer_bind_port(GstElement *element, int port) {
    GObject *obj = G_OBJECT(element);
    g_object_set(obj, "bind-port", port, NULL);
}

void gstreamer_set_ip(GstElement *element, char *host, int port) {
    GObject *obj = G_OBJECT(element);
    g_object_set(obj, "host", host, NULL);
    g_object_set(obj, "port", port, NULL);
}
